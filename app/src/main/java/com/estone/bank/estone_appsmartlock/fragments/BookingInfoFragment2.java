...
import com.estone.bank.estone_appsmartlock.adapters.BookingInfoAdapter;
 ...
 * interface.
 */
public class BookingInfoFragment2 extends Fragment implements View.OnClickListener, BookingInfoAdapter.OnItemClickedListener, BookingInfoAdapter.OnItemLongClickedListener, BookingInfoAdapter.OnItemViewClickListener {
    ...
    private List<User_AllDevices2.InfosBean> mInfos;
    ...
    public void OnItemDeleteClicked(int position) {
        ...
        User_AllDevices2.InfosBean infosBean = mInfos.get(position);
        String devId = infosBean.getDevId();
        Call<ResponsHead> removebookinformation =
                HttpManager.getInstance().getHttpClient().
                        removebookinformation(Contants.ACTIONID05,
                                devId, infosBean.getIdNumber(), infosBean.getCheckInStart(),
                                infosBean.getCheckInEnd());
            ...
            public void onResponse(Call<ResponsHead> call, Response<ResponsHead> response) {
                if (response.body() != null) {
                    if (response.body().getErrorCode() == 0) {
                        ...
                        Toast.makeText(getContext(), R.string.delect_success, Toast.LENGTH_LONG).show();
                        loadBookInformation();
                    } else {
                        Toast.makeText(getContext(), response.body() + response.body().getErrorMsg(), Toast.LENGTH_LONG).show();

                    }
                } else {
                    Toast.makeText(getContext(), R.string.delect_failed + ",ErrorCode==" + response.body(), Toast.LENGTH_LONG).show();
                }
            }
    ...
    public void OnItemEditClicked(int position) {
        ...
        bundle.putSerializable("User_AllDevices2InfosBean", infosBean);
    ...
    public void onItemlongClickedListeners(int position) {
        ...
        User_AllDevices2.InfosBean infosBean1 = mInfos.get(position);
        boolean clicked = infosBean1.isClicked();
        if (clicked) {
            for (int i = 0; i < mInfos.size(); i++) {
                ...
                if (i == position) {
                    ...
                    User_AllDevices2.InfosBean infosBean2 = mInfos.get(i);
                    infosBean2.setLongcheck(false);
                }
            }
            mAdapter.notifyDataSetChanged();
        } else {
            ToastUtils.showToast(getActivity(), "不可编辑");
        }
    ...
    public void OnItemViewClick(int posstion) {
        ...
        for (int i = 0; i < mInfos.size(); i++) {
            User_AllDevices2.InfosBean infosBean = mInfos.get(i);
    ...
    public static BookingInfoFragment2 newInstance(Bean_AllDevices.InfosBean sendinfosBean) {
        BookingInfoFragment2 fragment = new BookingInfoFragment2();
    ...
    private void initview(View view) {
        ...
        if (mAdapter == null) {
            mAdapter = new BookingInfoAdapter(getActivity());
        }
    ...
    private void loadBookInformation() {
        mAnimaition.start();
        bookinfor_anim.setVisibility(View.VISIBLE);
        Call<User_AllDevices2> User_AllDevices2Call =
                HttpManager.getInstance().getHttpClient().queryUserdevice
                        // 查询前后三天半
                                (Contants.ACTIONID14, MyApplications.getRegistXMId(), System.currentTimeMillis() - 302399000L,
                                        System.currentTimeMillis() + 302399000L, mSendinfosBean.getDevId());
        ...
        User_AllDevices2Call.enqueue(new Callback<User_AllDevices2>() {
            @Override
            public void onResponse(Call<User_AllDevices2> call, Response<User_AllDevices2> response) {
                if (response.body() != null) {
                    User_AllDevices2 body = response.body();
            ...
            @Override
            public void onFailure(Call<User_AllDevices2> call, Throwable t) {
                mAnimaition.stop();
                bookinfor_anim.setVisibility(View.GONE);
            }
        });
