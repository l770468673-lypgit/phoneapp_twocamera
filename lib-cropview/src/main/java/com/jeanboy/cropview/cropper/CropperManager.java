    ...
    public void pickFromCamera() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            cropperHandler.getActivity().startActivityForResult(new Intent(MediaStore.ACTION_IMAGE_CAPTURE).
                    putExtra(MediaStore.EXTRA_OUTPUT, uri), CropperParams.REQUEST_PICK_CAMERA);    // 相机
        } else {
        ...
        if (resultCode == Activity.RESULT_CANCELED) {
            ...
            switch (requestCode) {
                case CropperParams.REQUEST_PICK_CAMERA://相机拍完照回调处理，去裁切
                    ...
                    break;
                case CropperParams.REQUEST_PICK_IMAGE: //  相册回调
                    if (result != null) {
                        cropperHandler.getActivity().startActivityForResult(new Intent(cropperHandler.getActivity(), CropActivity.class)
                    ...
                    break;
                case CropperParams.REQUEST_KITKAT_PICK_IMAGE: //  相册回调
                    if (result != null) {
                        cropperHandler.getActivity().startActivityForResult(new Intent(cropperHandler.getActivity(), CropActivity.class)
                    ...
                    break;
                case CropperParams.REQUEST_CROPPED: // 裁剪回调
                    if (result != null) {
