    ...
    public void onStartThread() {
        synchronized (mLock) {
            if (null == mRecordThread) {
                FunLog.d(TAG,"mRecordThread=="+mRecordThread);
    ...
    class AudioRecordThread extends Thread {
        ...
        public boolean Start() {
            ...
            if (null == mAudioRecord || mAudioRecord.getState() == AudioRecord.STATE_UNINITIALIZED) {
                FunLog.d(TAG, "mAudioRecord=====" + mAudioRecord);
                FunLog.d(TAG, " mAudioRecord.getState()=====" + mAudioRecord.getState());
                FunLog.d(TAG, " AudioRecord.STATE_UNINITIALIZED=====" + AudioRecord.STATE_UNINITIALIZED);
        ...
        public void run() {
            ...
            while (!mThreadExitFlag) {
                ...
                if (AudioRecord.ERROR_INVALID_OPERATION != readsize
                    ...
                    FunLog.d(TAG, "requestDeviceSendTalkData=====" + mFunDevice);
                    FunLog.d(TAG, "audiodata=====" + Arrays.toString(audiodata));
                    FunLog.d(TAG, "readsize=====" + readsize);
                } else {
                    SystemClock.sleep(5);
                }
            }
            FunLog.d(TAG, "mThreadExitFlag=====" + mThreadExitFlag);
