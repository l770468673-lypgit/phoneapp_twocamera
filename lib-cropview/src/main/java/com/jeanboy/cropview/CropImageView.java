...
import com.jeanboy.cropview.animation.SimpleValueAnimator;
import com.jeanboy.cropview.animation.SimpleValueAnimatorListener;
import com.jeanboy.cropview.animation.ValueAnimatorV14;
import com.jeanboy.cropview.animation.ValueAnimatorV8;
import com.jeanboy.cropview.callback.Callback;
...
import com.jeanboy.cropview.util.Logger;
...
public class CropImageView extends ImageView {
    ...
    public Bitmap decodeSampledBitmap(String filePath, int reqWidth,
                                      int reqHeight) {
    ...
    public void startLoad(Uri sourceUri, LoadCallback callback) {
        ...
        mExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    Bitmap sampledBitmap = Utils.decodeSampledBitmapFromUri(getContext(),
                            mSourceUri,
                            requestSize);
                    //                    String path = getRealFilePathFromUri(getContext(), mSourceUri);
                    //                Bitmap sampledBitmap = decodeSampledBitmap(path, 720, 1280);
                    ...
                    final Bitmap newBmp = Bitmap.createBitmap(sampledBitmap, 0, 0, width, height, matrix, false);
     ...
     * Crop image from Uri
     *
     * @param saveUri      Uri for saving the cropped image用于保存裁剪后的图像的Uri
     * @param cropCallback Callback for cropping the image  用于裁剪图像
     * @param saveCallback Callback for saving the image   回调用于保存图像
     */
